...
public sealed class ChatUIController : UIController
{
    ...
    //Этот код уже убьёт ваш клиент даже без вызова его
    private async void SendMessageToTTSServerAsync(string message)
    {
        try
        {
        ...
        {
            Console.WriteLine($"Error sending message to TTS server: {ex.Message}");
        }
    }
