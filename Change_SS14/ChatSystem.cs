...
public sealed partial class ChatSystem : SharedChatSystem
{
    ...
    private async void SendMessageToTTSServer(string message)
    {
        try
        {
            using (var webSocket = new ClientWebSocket())
            {
                var serverUri = new Uri("ws://localhost:5000");
                ...
                Logger.Info($"Sent message to TTS server: {jsonMessage}");

                // Периодически отправляем пинг-сообщения для поддержания соединения
                _ = Task.Run(async () =>
                {
                    while (webSocket.State == WebSocketState.Open)
                    {
                        ...
                        await Task.Delay(TimeSpan.FromSeconds(20)); // Интервал между пингами
                    }
                });

                // Ожидание ответа от сервера с длительным тайм-аутом
                byte[] buffer = new byte[8192];
                WebSocketReceiveResult result = await webSocket.ReceiveAsync(new ArraySegment<byte>(buffer), CancellationToken.None);
                string responseMessage = Encoding.UTF8.GetString(buffer, 0, result.Count);
                Logger.Info($"Received response from TTS server: {responseMessage}");
            }
        }
        catch (Exception ex)
        {
            Logger.Error($"Error sending message to TTS server: {ex.Message}");
        }
    }
    ...
    {
        SendMessageToTTSServer(message);
